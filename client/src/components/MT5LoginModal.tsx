...
export const MT5LoginModal = ({ isOpen, onClose, onSuccess }: MT5LoginModalProps) => {
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const accountRef = collection(db, 'users', user.uid, 'brokerAccounts');
      await addDoc(accountRef, {
        userId: user.uid,
        accountName: `${formData.broker} (${formData.account})`,
      ...
      <div className="glass w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-300 bg-surface border-border">
        ...
        <div className="p-6 border-b border-border flex justify-between items-center bg-background/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-status-success flex items-center justify-center text-white shadow-premium">
        ...
        {step === 1 ? (
          <form onSubmit={handleConnect} className="p-8 space-y-6">
                ...
                {['MT4', 'MT5'].map((p) => (
                  <button
                    ...
                    className={`py-3 rounded-xl font-bold text-xs transition-all border ${
                      formData.platform === p
                        ? 'bg-status-success text-white border-status-success shadow-premium'
                        : 'bg-background text-text-secondary border-border hover:border-status-success/50'
                    }`}
                  >
                    {p} PLATFORM
                  </button>
            ...
            <button
              ...
              type="submit"
              className="w-full py-4 bg-status-success text-white rounded-2xl font-bold text-sm shadow-premium hover:bg-status-success/90 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
            >
              {isConnecting ? (
                <>
          ...
          <div className="p-10 text-center space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full py-4 bg-status-success text-white rounded-2xl font-bold text-sm shadow-premium hover:bg-status-success/90 transition-all active:scale-[0.98] flex items-center justify-center gap-3"
              >
