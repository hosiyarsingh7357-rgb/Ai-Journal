...
const calendarEvents = [
  {
    ...
    impact: 'High Impact',
    impactColor: 'text-status-danger bg-status-danger/10',
    actual: '--',
    forecast: '185K',
    previous: '203K',
    flag: '🇺🇸'
  },
  {
    ...
    impact: 'Med Impact',
    impactColor: 'text-status-warning bg-status-warning/10',
    actual: '2.4%',
    actualColor: 'text-status-success',
    forecast: '2.5%',
    previous: '2.6%',
    flag: '🇪🇺'
  },
  {
    ...
    impact: 'Low Impact',
    impactColor: 'text-text-secondary bg-surface-muted',
    flag: '🇺🇸'
  }
];
                ...
                <table className="w-full text-left border-separate border-spacing-y-4 px-4">
                  ...
                  <tbody className="text-sm">
                    {marketPulseData.map((item, index) => (
                      <tr key={index} className="group hover:bg-surface-muted/50 transition-colors">
                        <td className="px-6 py-4 first:rounded-l-xl last:rounded-r-xl">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-bold text-text-primary leading-tight">{item.symbol}</p>
                              <p className="text-[10px] text-text-secondary font-medium">{item.name}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-right font-bold text-base text-text-primary">{item.price}</td>
                        ...
                        <td className="px-6 py-4">
                          <div className="h-8 w-32 mx-auto overflow-hidden">
                            <Sparkline data={item.trend} color={item.isPositive ? 'var(--status-success)' : 'var(--status-danger)'} />
                          </div>
                        </td>
                        <td className="px-6 py-4 text-right text-text-secondary font-medium">{item.volume}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <Card className="p-6 flex flex-col justify-between h-48">
                <div>
                  ...
                  <div className="mt-6 flex flex-col gap-2">
                    ...
                    <p className="text-[10px] text-text-secondary mt-2 italic text-center">Based on 12k institutional positions</p>
                  </div>
                </div>
              </Card>
            </div>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-8">
            ...
            <Card className="overflow-hidden flex flex-col !p-0">
              {calendarEvents.map((event, index) => (
                <div key={index} className={`p-5 group hover:bg-surface-muted/50 transition-colors ${index !== calendarEvents.length - 1 ? 'border-b border-border' : ''}`}>
                  <div className="flex items-start justify-between">
                    <div className="flex gap-4">
                      <div className="text-center w-10">
                        <p className="text-xs font-extrabold text-text-primary">{event.time}</p>
                        <p className="text-[10px] text-text-secondary">{event.timezone}</p>
                      </div>
                      <div>
                        ...
                        <h4 className="text-sm font-bold text-text-primary group-hover:text-brand-primary transition-colors">{event.event}</h4>
                      </div>
                    </div>
                    <span className={`px-2 py-0.5 rounded text-[9px] font-extrabold ${event.impact.toLowerCase().includes('high') ? 'bg-status-danger/10 text-status-danger' : event.impact.toLowerCase().includes('med') ? 'bg-status-warning/10 text-status-warning' : 'bg-surface-muted text-text-secondary'} tracking-tighter uppercase`}>
                      {event.impact}
                    </span>
                  </div>
                  ...
                  {event.actual && (
                    <div className="mt-4 grid grid-cols-3 gap-2">
                      <div className="bg-background/50 p-2 rounded">
                        <p className="text-[9px] text-text-secondary font-bold">ACTUAL</p>
                        <p className={`text-xs font-bold ${event.actualColor ? 'text-status-success' : 'text-text-primary'}`}>{event.actual}</p>
                      </div>
                      <div className="bg-background/50 p-2 rounded">
                        <p className="text-[9px] text-text-secondary font-bold">FORECAST</p>
                        <p className="text-xs font-bold text-text-primary">{event.forecast}</p>
                      </div>
                      <div className="bg-background/50 p-2 rounded">
                        <p className="text-[9px] text-text-secondary font-bold">PREVIOUS</p>
                        <p className="text-xs font-bold text-text-primary">{event.previous}</p>
                      </div>
                    </div>
                  )}
                </div>
              ))}
              <button className="m-4 py-3 bg-surface text-brand-primary text-[11px] font-extrabold uppercase tracking-widest rounded-lg hover:bg-brand-primary hover:text-white transition-all border border-border">
                VIEW FULL CALENDAR
              </button>
            </Card>

            {/* Breaking News Snippet */}
            <Card className="bg-brand-primary/10 border-brand-primary/20 text-text-primary p-6 relative overflow-hidden shadow-premium">
              <div className="relative z-10">
                ...
                <div className="mt-6 flex items-center justify-between border-t border-border pt-4">
                  <span className="text-[10px] text-text-secondary">2 MINUTES AGO</span>
                  <ChevronRight className="w-4 h-4 text-brand-primary hover:text-text-primary cursor-pointer" />
                </div>
              </div>
